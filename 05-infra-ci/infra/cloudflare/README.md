...
3. Copy `05-infra-ci/infra/cloudflare/wrangler.ingress.example.toml` into `01-ingress-worker/wrangler.toml`.
...
When the dashboard app exists, copy `05-infra-ci/infra/cloudflare/wrangler.dashboard.example.toml` into `04-dashboard/wrangler.toml` or wire the same values into the Cloudflare Pages dashboard.
